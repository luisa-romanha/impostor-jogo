...
**Impostor de Palavras** é um jogo mobile-first de dedução social (estilo "party game") desenhado para ser jogado localmente em grupo, utilizando apenas um único dispositivo (telemóvel/smartphone) que é passado de mão em mão entre os jogadores.

**O Objetivo:** Todos os jogadores recebem a mesma palavra secreta baseada numa categoria, exceto um jogador sorteado aleatoriamente: o **Impostor**. O objetivo dos jogadores normais é descobrir quem é o impostor através de discussão. O objetivo do impostor é disfarçar, tentar adivinhar a palavra pelo contexto da conversa e não ser votado.
...
- Limite de **15 caracteres** para os nomes para evitar quebra de layout no ecrã.
...
O jogo funciona num modelo de **"Máquina de Estados" (State Machine)**, garantindo que os jogadores não podem acidentalmente espreitar a vez do outro.
...
3. **Fase de Revelação (Passagem do Telemóvel)**
   - O ecrã mostra um aviso pulsante: *"Passe o telemóvel para [Nome]"*.
   - O jogador confirma a identidade (*"Eu sou [Nome]"*).
   - Toca no botão para revelar a palavra (ou o estatuto de Impostor).
   - Clica em **"Esconder e Passar"** antes de entregar ao próximo.

4. **Fase de Discussão** *(Fora do ecrã)*
   - Os jogadores conversam e fazem perguntas subtis uns aos outros sobre a palavra.

5. **Fase de Votação Secreta**
   - Semelhante à fase de revelação, o telemóvel roda entre os jogadores.
...
A matemática dos resultados no final de cada ronda funciona da seguinte forma:

### 🔴 O Impostor é Apanhado (Derrota do Impostor)
...
| **Gestão de Estado** | `useState` | Controla o fluxo de ecrãs (`stage`), dados da ronda e pontuações globais |
